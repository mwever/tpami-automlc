...
public class StatisticsUtil {
	...
	public static double wilcoxonSignedRankSumTestP(final double[] sampleA, final double[] sampleB) {
		Set<Integer> indicesWithNans = new HashSet<>();
		IntStream.range(0, sampleA.length).filter(x -> Double.isNaN(sampleA[x]) || Double.isNaN(sampleB[x])).forEach(indicesWithNans::add);
//		System.out.println(indicesWithNans + " " + Arrays.toString(sampleA) + " " + Arrays.toString(sampleB));
		...
		return new WilcoxonSignedRankTest(NaNStrategy.REMOVED, TiesStrategy.AVERAGE).wilcoxonSignedRankTest(cleanedSampleA, cleanedSampleB, false);
	}
