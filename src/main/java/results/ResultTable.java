...
public class ResultTable {
	...
	private static final String TEST_FILE_NAME = "data/result_table_test_" + measureIx + ".data";
	private static final String INTERNAL_FILE_NAME = "data/result_table_internal_" + measureIx + ".data";
	private static final String RANDOM_INTERNAL_FILE_NAME = "data/result_table_random_internal.data";

	private static final Map<String, String> replacements = new HashMap<>();

	private static final boolean REFRESH_DATA_FROM_DB = false;
	...
	private static void refreshKVStoreFiles() throws SQLException, IOException {
		// Load test data
		...
		col.serializeTo(new File(TEST_FILE_NAME));
		...
		if (INCLUDE_INTERN) {
			...
			internal.serializeTo(new File(INTERNAL_FILE_NAME));
			...
			randomEval.serializeTo(new File(RANDOM_INTERNAL_FILE_NAME));
		}
	}

	public static void main(final String[] args) throws SQLException, IOException {
		loadReplacements();
		if (REFRESH_DATA_FROM_DB) {
			refreshKVStoreFiles();
		}

		KVStoreCollection col = new KVStoreCollection(FileUtils.readFileToString(new File(TEST_FILE_NAME)));
		...
		if (INCLUDE_INTERN) {
			KVStoreCollection internal = new KVStoreCollection(FileUtils.readFileToString(new File(INTERNAL_FILE_NAME)));
			internal = new KVStoreCollection(internal.stream().filter(x -> x.containsKey("measure") && x.getAsString("measure").equals(measures[measureIx])).map(x -> {
			...
			KVStoreCollection randomEval = new KVStoreCollection(FileUtils.readFileToString(new File(RANDOM_INTERNAL_FILE_NAME)));
			ObjectMapper mapper = new ObjectMapper();
			for (IKVStore store : randomEval) {
				store.put("algorithm", replacements.get(store.getAsString("algorithm")) + "_int");
				store.put("finalScore", ValueUtil.round(mapper.readTree(store.getAsString("evaluation_report")).get(measures[measureIx] + "_mean").asDouble(), 2));
			}
