...
import ai.libs.mlplan.multilabel.mekamlplan.IMekaPipelineFactory;
...
public class MekaPipelineFactory implements IMekaPipelineFactory {
	...
	private List<String> getOptionsForParameterValues(final ComponentInstance ci) {
		...
		for (Entry<String, String> parameterValue : ci.getParameterValues().entrySet()) {
			...
			if (parameterValue.getKey().toLowerCase().contains("activator") || parameterValue.getValue().equals("false")) {
			...
			if (param.getDefaultDomain() instanceof NumericParameterDomain && ((NumericParameterDomain) param.getDefaultDomain()).isInteger() && param.getDefaultValue().toString().contains(".")) {
				isDefault = ((int) Double.parseDouble(param.getDefaultValue().toString())) == ((int) Double.parseDouble(parameterValue.getValue()));
			} else {
				isDefault = param.getDefaultValue().toString().equals(parameterValue.getValue());
			}
